...
const CodeBlock: React.FC<CodeBlockProps> = ({ language, code }) => {
  ...
  return (
    <div className="my-4 rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700">
      ...
      <div className="relative overflow-x-auto custom-scrollbar" style={{ background: bgColor }}>
        {Highlighter ? (
          <div className="wrap-break-word">
            <Highlighter
              ...
              PreTag="div"
              customStyle={{ margin: 0, borderRadius: 0, background: 'transparent', color: fgColor, padding: '0.75rem' }}
            >
              {code.replace(/\n$/, '')}
            </Highlighter>
          </div>
        ) : (
          <pre className="p-3 wrap-break-word" style={{ color: fgColor, margin: 0, background: 'transparent' }}>{code}</pre>
        )}
        {showCollapse && (
