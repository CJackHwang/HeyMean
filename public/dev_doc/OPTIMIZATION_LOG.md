...
##### 2.4 自适应与交互恢复（2024-12）
- 恢复原版 header / main 布局，移动端重新提供抽屉关闭按钮
- 新建笔记改为“立即写入数据库”，确保随时可恢复、可删除
- 未保存变更弹窗恢复为“保存 / 丢弃 / 取消”三按钮流，匹配老版本体验
- 删除/重命名/固定等操作统一经 `useNoteActions` 执行，避免 UI 分支逻辑
- 新增 BUGFIX_NOTES_UI.md 记录，便于后续排查

#### 优化成果
- `NotesView.tsx`: 537行 → 307行 **减少 43%**（在恢复交互后稳定在 350± 行）
- **性能提升**：
